...
| --- | --- | --- | --- |
| **шість, як відвантажено** | **0.822** | **1 крок** | 0.005 |
| лише `brenner` | 0.747 | 5 кроків | 0.090 |
| лише `wavelet` | 0.743 | 5.5 | 0.090 |
| лише `tenengrad` | 0.729 | 5 | 0.089 |
| лише `laplacian` | 0.720 | 7 | 0.165 |
| лише `edge_width` | 0.705 | 1 | 0.179 |
| лише `fourier` | 0.690 | 6.5 | 0.250 |
...
Ще різкіша відмінність — **плато піка**: одна міра лишає плато завширшки 5–7
кроків, тобто не локалізує максимум узагалі; шість лишають 1 крок. І насичення
в одиничних мір у 18–50 разів вище.
...
Чого їй бракує: обмеженої шкали 0–1, порівнюваності між сценами, впевненості й
того, що зливати. Для пошуку максимуму в межах одного потоку **шкала не
потрібна** — потрібен лише порядок, і сира міра його дає краще за конвеєр. Чи
варті перелічені властивості 0.023 — це вибір, а не наслідок.
