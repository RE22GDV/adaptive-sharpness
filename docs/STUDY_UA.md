...
Коротко — рангова кореляція з еталона перейшла з **виразно відʼємної** до
**0.949**, а частка хибно впорядкованих пар кроків — з **0.53** до **0.05**.
...
**Проти простого середнього перевага стабільна, але не доведена.** Поточна
конфігурація попереду на **обох** записах і при **обох** рецептах еталона — на
0.008…0.018 рангової кореляції, чотири виміри одного знаку. Але еталон несуть
лише два записи, і різниця менша за розкид між записами. Це **стабільний
напрям, а не встановлений ефект**, і так він і поданий.

---

## 5. Конвеєр гірший за свій найкращий вхід

Таблиця: [сигнали й конвеєри окремо](RESULTS.md#signals-and-pipelines-compared-separately).

Сира метрика `wavelet` дає **0.988**. Найкращий повний конвеєр — **0.951**.
**Нормалізація, злиття й фільтрація разом коштують близько 0.04 згоди з
еталоном** порівняно з простим використанням найкращої сирої міри.

Це не привід відвантажувати сиру міру: у неї немає обмеженої шкали, порівнюваності
між сценами, впевненості й нічого зливати, а саме порівняння — офлайнове на
запису, весь діапазон якого відомий наперед. Але воно **обмежує те, що дає вся
ця машинерія**, і чесне формулювання таке: конвеєр виміняв точність на
властивості, потрібні живій системі, а не покращив свої входи.
