...
1. **Фіксація шкали — найбільший ефект у проєкті.** Без неї рангова кореляція
   з еталоном 0.50; з нею — 0.97 (Д11). Але це ефект **лише за одним
   критерієм**: на розрізнення сусідніх кроків вона додає всього +0.012.
   Фіксація виправляє **напрямок**, злиття й фільтр дають **роздільність**
   (+0.125 adj разом). Компоненти роблять різні роботи —
   [розбір](STUDY_UA_REPORT.md#91-метод-називається-не-на-честь-того-що-в-ньому-працює).
2. **Переконливої різниці між адаптивними й фіксованими вагами не
   встановлено.** 95% інтервал — [−0.046, +0.029] при оцінці −0.005, тобто
   корпус не здатен розділити ці варіанти; це **не доказ їх рівноцінності**.
   Чому саме так, видно у вагах: вони майже не рухаються (Д07, Д12).
3. **Уся втрата припадає на *цей* спосіб потокової нормалізації, а не на
   шкалу як таку.** Офлайнове масштабування безкоштовне; ковзні процентилі з
   фіксацією коштують −0.193 adj, і злиття з фільтром повертають дві третини
   (Д09). Що інший онлайновий нормалізатор коштував би стільки ж, не
   перевірялося.
...
Тобто **переконливої різниці не встановлено** — ні в кращий, ні в гірший бік.
Твердження §5.3, що адаптивність «програє за adj», треба замінити на «не
встановлено».

**Це не те саме, що рівноцінність.** Інтервал каже, що цей корпус не здатен
розділити варіанти, а не що вони однакові. Справжня різниця може лежати
будь-де в межах ±0.04, і щоб її знайти, потрібні не інші обчислення, а більше
незалежних сцен.
...
- **Уся втрата — це перехід на *цю* потокову нормалізацію:** −0.193 adj і
  −0.039 Спірмена. Виміряно один конкретний спосіб — ковзні процентилі з
  логарифмом, автофіксацією та логістичним відображенням. Це **не нижня межа
  для потокової роботи взагалі**: інший онлайновий оцінювач діапазону міг би
  втратити менше, і жоден такий тут не перевірявся.
...
**Розкид 0.591…0.971 по сітці походить від одного запису й одного
параметра.** Спірмен визначений лише на двох точкових записах, і вони обидва
довгі (1781 і 1690 кадрів), тож фіксація спрацьовує на них за **будь-якого**
набору порогів. Частота спрацювання по всіх восьми записах на Спірмена тому не
впливає взагалі — вона змінюється лише там, де Спірмен не визначений.

Розклад за записами:

| Запис | Кадрів | Спірмен по всій сітці |
| --- | --- | --- |
| Точкове джерело 1 | 1781 | 0.950…0.961 — **параметри не змінюють нічого** |
| Точкове джерело 2 | 1690 | 0.232…0.981 — увесь розкид тут |

На другому записі розкид пояснює **терпіння** (`auto_freeze_patience`), і лише
воно:

| Терпіння | Спірмен на точковому джерелі 2 |
| --- | --- |
| 60 | 0.870…0.981 |
| 120 | 0.233…0.979 |
| **240** | **0.232…0.356** |

Мінімальна історія не впливає (0.232…0.981 за будь-якого з трьох значень).
Довге терпіння змушує фіксацію спрацювати пізно, і зафіксована шкала на цьому
записі виявляється непридатною.

Тому коректне формулювання обережніше, ніж хотілося б: **наявність фіксації
вирішальна** (0.50 проти 0.97), **а залежність від порогів спостережена на
одному записі з двох** і зводиться до одного параметра. Це підстава не ставити
терпіння завеликим, але не підстава стверджувати загальну залежність — вибірка
для такого твердження складається з двох сцен.

> **Виправлено після звірки з первинними звітами.** Попередня редакція
> пояснювала розкид часткою записів, на яких спрацювала фіксація, і наводила
> таблицю з чотирьох груп. Після перерахунку виявилося, що два набори
> (`min480_pat60_*`) спрацьовують лише на 6 записах із 8 і при цьому дають
> Спірмена 0.965 — тобто зв'язок був позірним. Причина проста: обидва записи,
> на яких Спірмен взагалі визначений, входять до тих, де фіксація спрацьовує
> завжди.
...
| Розмір | Найкращий набір | adj (8) | Спірмен (2) |
| --- | --- | --- | --- |
| 1 | Бреннер | 0.7469 | 0.9304 |
| 2 | Фур'є + ширина меж | 0.8023 | 0.9608 |
| 3 | Лапл. + вейвл. + Фур'є | 0.8181 | 0.9549 |
| 4 | Лапл. + Бренн. + вейвл. + Фур'є | 0.8241 | 0.9540 |
| **5** | **без Тененграда** | **0.8348** | **0.9682** |
| 6 | усі шість (поточний) | 0.8218 | 0.9653 |

> Колонку часу прибрано навмисно. Дослідження Д07–Д20 рахувалися на робочій
> станції, і час там відтворюється з точністю близько 15 % між прогонами: той
> самий набір із пʼяти метрик дав 4.68 мс в одному прогоні й 4.77 мс у
> наступному. Для будь-яких тверджень про швидкодію треба брати числа з
> вимірювань на Raspberry Pi 5 ([RESULTS.md](RESULTS.md)).
...
| Варіант | adj (8) | Спірмен (2) | Відносний час |
| --- | --- | --- | --- |
| **320, весь кадр (поточний)** | **0.8218** | **0.9653** | 1.0× |
| 480, весь кадр | 0.8207 | 0.8568 | 1.5× |
| 640, весь кадр | 0.8100 | 0.8310 | 2.4× |
| 160, весь кадр | 0.7972 | 0.9311 | 0.7× |
| 320, центр 50 % | 0.7900 | 0.7985 | 0.9× |

Час поданий відносно поточного варіанта: абсолютні мілісекунди тут виміряні на
робочій станції й між прогонами розходяться приблизно на 15 %, тоді як
відношення між варіантами в одному прогоні стійке.
...
**Як трактувати.** Весь діапазон adj по всіх множниках — 0.814…0.830, тобто
розмах **0.016**: менше за половину роздільної здатності корпусу
([Д07](#д07-невизначеність-парних-різниць) дає ±0.04). **Коефіцієнти майже не
важать.** Це узгоджується з Д12: ваги й так майже не
...
Усі рядки — вісім записів, і жоден не перевищує свого бюджету.

| Стратегія / бюджет | Оцінювань | Знайшов максимум сигналу | У межах 1 кроку від еталона |
| --- | --- | --- | --- |
| Сходження, 4 | 3.8 | 0.51 | 0.53 |
| Сходження, 6 | 4.9 | 0.70 | 0.73 |
| Сходження, 10 | 5.6 | 0.85 | **0.97** |
| Сходження, 16 | 5.7 | 0.87 | **0.97** |
| **Тернарний, 4** | **4.0** | 0.50 | **1.00** |
| Тернарний, 8 | 7.5 | 0.88 | **1.00** |
| Тернарний, 16 | 8.5 | 0.88 | **1.00** |

**Як трактувати.** Сигнал придатний для пошуку, і дешевше, ніж здавалося:
тернарний пошук потрапляє в межі одного кроку від еталона у **100 %** запусків
уже за **4 оцінки**. Сходження вгору дешевше за крок, але залежить від старту
й виходить на 97 % лише при бюджеті 10.

Зверніть увагу на розбіжність двох колонок у тернарного пошуку: він знаходить
**власний максимум сигналу** лише у 50–88 % випадків, але в межі одного кроку
від еталона потрапляє завжди. Тобто там, де він «промахується» повз argmax,
він промахується на один крок — а еталон точніше й не розрізняє.

Залишкова помилка 1.0 крок відповідає роздільності еталона, встановленій
окремо в [§5.8](STUDY_UA_REPORT.md#58-чутливість-опорного-вимірювання): зміна
рецепта зсуває опорний максимум приблизно на крок. Сама величина похибки цього
б не довела.

> **Виправлено після рецензії.** Перша версія цього дослідження мала два
> дефекти. Тернарний пошук **перевищував власний бюджет** — прохід по залишку
> діапазону після звуження не враховував ліміт, тож рядок «бюджет 6» показував
> 10 оцінювань. І бюджети включали `кроки × 2`, що різне для різних записів,
> тож рядок «бюджет 30» був середнім за трьома записами поруч із рядком
> «бюджет 6» за вісьмома — їхні частки успіху не можна було порівнювати.
>
> Обидва виправлено, інваріант закріплено тестами
> ([tests/test_programme.py](../tests/test_programme.py)), дослідження
> перераховано з чистого дерева. Виправлені числа **кращі** за опубліковані:
> 4 оцінки замість 8.5 на ту саму стовідсоткову точність.
...
Д07, не відокремлюваний від нуля), а не властивість критеріїв. Обидва
можна використовувати далі.

**Похибка піка майже не корелює з рештою** (0.18 і 0.21). Це означає, що вона
**міряє щось інше**, а не що вона непридатна: вона відповідає на питання «чи в
тому місці максимум», тоді як `adj` і Спірмен — на питання «чи правильний
порядок». Для ранжування конфігурацій на цьому корпусі вона слабка, бо її
значення майже завжди 0.5…1.5 кроку й розкид малий.

Окремо: похибка в один крок **сама собою не доводить**, що це межа точності
еталона. Такий висновок спирається на [Д05/§5.8](STUDY_UA_REPORT.md#58-чутливість-опорного-вимірювання),
де зміна рецепта справді зсуває опорний максимум приблизно на крок. Без цього
одна лише величина похибки не відрізняє «еталон не бачить точніше» від
«модель промахується на крок».
