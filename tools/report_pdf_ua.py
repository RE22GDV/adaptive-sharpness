...
def build(data: Data, out: Path) -> Path:
    ...
    story += [
        ...
        P(
            ...
            "числа: рангова кореляція з фізичним еталоном падає до "
            f"{data.agg(d11, 'off', 'spearman_gt'):.2f} — рівень випадкового "
            f"впорядкування. З фіксацією — "
            f"{data.agg(d11, 'min240_pat120_stab0.05', 'spearman_gt'):.2f}.", "body",
        ),
        P(
